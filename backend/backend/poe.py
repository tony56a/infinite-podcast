...
def get_latest_message(bot):
    ...
    while True:
        time.sleep(2)
        if (datetime.now() - start_timestamp).total_seconds() > 180:
            logger.error("timed out waiting for Poe response")
            return None
        response = requests.post(url, headers=headers, json=data)
        response_json = response.json()
        text = response_json["data"]["chatOfBot"]["messagesConnection"]["edges"][-1][
            "node"
        ]["text"]
        state = response_json["data"]["chatOfBot"]["messagesConnection"]["edges"][-1][
            "node"
        ]["state"]
        author_nickname = response_json["data"]["chatOfBot"]["messagesConnection"][
            "edges"
